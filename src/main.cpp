...
// NOTE: this should be the last include
#include "logger.h"
...
int main(int argc, char* argv[]) {
    ...
    using interpreter_ptr       = std::unique_ptr<xeus_clickhouse::interpreter>;
    interpreter_ptr interpreter = std::make_unique<xeus_clickhouse::interpreter>();
    ...
    if (!file_name.empty()) {
        ...
        // clang-format off
        log(logINFO) <<
            "Starting xeus-clickhouse kernel...\n\n"
        ...
        xeus::xkernel kernel(xeus::get_user_name(), std::move(interpreter), std::move(hist), nullptr, xeus::make_xserver_shell_main);
